...
export function prepareDay(state,plan){
 if(!plan)return;
 state.campaign=true;state.dayPlan=plan;state.conditions=CONDITIONS[plan.weather]||state.conditions;state.training=plan.mode==='training';if(state.training)state.shiftEnd=state.minute+10080;
 if(plan.schedule){state.schedule=structuredClone(plan.schedule);state.next=0;}
  ...
  else{const live=state.schedule.filter(c=>!c.noDispatch);if(live.length&&random()<.6){const i=state.schedule.indexOf(live.at(-1));state.schedule[i]={...EXTRA_MISSIONS[random()<.5?0:1],id:undefined,at:state.schedule[i].at,seed:plan.seed+991};}if(plan.handover&&live[0]){live[0].at=state.minute+.1;live[0].handover=true;}if(plan.weather==='crowd'){const event=state.schedule.find(c=>c.type==='SUAP'&&!c.noDispatch);if(event){event.setting='stadium';event.name='Malaise pendant le match';event.catalogId=undefined;event.scene='medical';}}state.schedule.sort((a,b)=>a.at-b.at);}
  state.dayPlan={...plan,schedule:structuredClone(state.schedule)};
 }
...
export function dayResult(state){const calls=state.calls.filter(c=>c.at>=state.shiftStart),done=calls.filter(c=>c.status==='closed'),delays=calls.filter(c=>c.firstArrival!=null).map(c=>c.firstArrival-c.at);const average=delays.length?delays.reduce((n,v)=>n+v,0)/delays.length:0;return{at:Date.now(),seed:state.dayPlan?.seed,completed:done.length,total:calls.length,average:Math.round(average*10)/10,coverage:!state.objectives?.find(o=>o.key==='coverage')?.broken};}
