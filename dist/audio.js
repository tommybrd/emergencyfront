let ctx,master,on=true,volume=.45,sampleUntil=0,pendingSound=null,sirenGain=0,heldGain=1;
...
export async function toggleSound(){on=!on;await ready();if(!on){pendingSound=null;oneShot.pause();oneShot.currentTime=0;stopPager();twoTone.pause();twoTone.currentTime=0;}return on;}
export function setVolume(v){volume=Math.max(0,Math.min(1,v));if(master)master.gain.value=on?volume:0;pager.volume=volume*.5;twoTone.volume=volume*(Date.now()<sampleUntil?1:sirenGain);oneShot.volume=volume*heldGain;}
export function stopPager(){pager.pause();pager.currentTime=0;}
export async function activateAudio(){if(!on)return;await ready();if(pendingSound){const type=pendingSound;pendingSound=null;sound(type);}}
...
export function syncSiren(active,gain=1,momentaryGain=1){sirenGain=Math.max(0,Math.min(1,gain));heldGain=Math.max(0,Math.min(1,momentaryGain));oneShot.volume=volume*heldGain;if(Date.now()<sampleUntil)return;twoTone.volume=volume*sirenGain;twoTone.loop=true;if(active&&on)sound('siren');else if(!twoTone.paused)stopSiren();}
