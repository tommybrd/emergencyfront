...
export function crewPanel(e,state){const crew=engineCrew(e,state);if(!crew.length)return '';const leader=crew.reduce((a,b)=>b.rank>a.rank?b:a);return `<section class="crewRoster" data-crew="${escapeHtml(e.id)}"><div class="crewHeading">Équipage · ${crew.length} personnel${crew.length>1?'s':''}</div><ul>${crew.map(p=>`<li><span class="crewRank rank${p.rank}" aria-hidden="true">${p.rank===0?'▰':'▰'.repeat(Math.min(3,p.rank))}</span><div><b>${escapeHtml(p.grade)} ${escapeHtml(p.name)}</b><small>${p.role==='captain'?'Chef de centre':p.role==='nurse'?'Soins':p.id===leader.id?'Chef d’agrès':''}</small></div></li>`).join('')}</ul></section>`;}
