...
export const canEngage=e=>e.external?e.status==='ready'&&!e.doctorAway:['ready','idle','returning'].includes(e.status)||e.kind==='VLCG'&&e.status==='departing'&&!!e.commuteDestination;
export function capability(e,c){
 if(e.kind==='VLI')return ['SUAP','AVP'].includes(c.type)||c.type==='INC'&&c.victimsKnown&&c.patients?.some(p=>!p.evacuated)?'nursing':null;
 ...
 if(e.kind==='EPA')return c.type==='INC'&&c.requires!=='CCF'?'aerial':null;
 ...
 if(e.kind==='VSAV')return ['SUAP','AVP'].includes(c.type)||c.type==='INC'&&c.victimsKnown&&c.patients?.some(p=>!p.evacuated)?'resolve':null;
