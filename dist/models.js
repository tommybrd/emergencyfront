...
export function person(parent,x,z,color='#394e66',firefighter=false){const g=new T.Group();g.position.set(x,0,z);parent.add(g);const body=box(g,.52,.73,.3,firefighter?'#323b39':color,0,1.06,0);box(g,.2,.63,.23,'#273039',-.15,.43,0);box(g,.2,.63,.23,'#273039',.15,.43,0);box(g,.14,.64,.18,firefighter?'#323b39':color,-.36,1.01,0);const arm=box(g,.14,.64,.18,firefighter?'#323b39':color,.36,1.01,0);const head=new T.Mesh(new T.SphereGeometry(.23,8,6),mat('#c99775'));head.position.y=1.65;g.add(head);if(firefighter){helmetF1(g);box(g,.54,.09,.32,'#dad474',0,.98,0);box(g,.22,.11,.25,'#dbd77c',-.15,.43,0);box(g,.22,.11,.25,'#dbd77c',.15,.43,0);cylinder(g,.13,.13,.6,'#a6afb0',0,1.05,-.25,8);arm.rotation.x=-1;}return g;}
export function vehicle(parent,kind='FPT',color='#bd292b',options={}){
 const g=kind==='VLCG'&&color==='#bd292b'&&!options.serviceCar?pickupModel(parent):kind==='VSAV'&&options.ambulanceModel==='man'?manAmbulance(parent,options):kind==='FPT'||kind==='CCF'||kind==='EPA'||kind==='CCGC'?truckModel(parent,kind,color,options):vanModel(parent,kind,color,options);return g;
}
...
if(kind==='FPT')pumpRear(g,back,color,options.lightPump);
