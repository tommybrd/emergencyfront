...
export function tree(parent,x,z,size=1,type=0){const g=new T.Group();g.position.set(x,0,z);g.scale.setScalar(size);size=1;parent.add(g);cylinder(g,.2*size,.32*size,3*size,'#6c5745',0,1.5*size,0,6);const palette=['#53754b','#718b50','#3e6b58','#89924c'];for(let i=0;i<3;i++){const m=new T.Mesh(new T.IcosahedronGeometry((1.65-i*.28)*size,1),mat(palette[type%4]));m.position.set(i===1?.65*size:i===2?-.55*size:0,(3.6+i*.6)*size,i===2?.5*size:0);m.scale.y=1.1;m.castShadow=true;g.add(m);}return g;}
...
if(ambulance){box(g,width+.1,2.05,3.85,red,0,1.75,back+1.925);box(g,width+.18,.12,3.98,'#f3f1e2',0,2.84,back+1.925);box(g,.85,.2,.85,'#edeedf',0,3.02,back+2.7);for(const side of[-1,1]){box(g,.035,.055,3.85,fluoro,side*(w+.07),2.68,back+1.925);box(g,.035,.045,3.85,fluoro,side*(w+.07),1.08,back+1.925);box(g,.045,1.65,.04,silver,side*(w+.08),1.75,-.85);box(g,.045,1.65,.04,silver,side*(w+.08),1.75,-1.78);for(const yy of[.94,2.57])box(g,.045,.04,.97,silver,side*(w+.08),yy,-1.31);box(g,.06,.18,.07,rubber,side*(w+.11),1.53,-1.63);const text=sign(g,'SECOURS ET ASSISTANCE AUX VICTIMES',2.6,.42,side*(w+.1),2.04,-1.3,red,'#fff9e6');text.rotation.y=side*Math.PI/2;}}
...
box(g,.6,.13,.035,'#e4e8de',0,car?.6:.72,front+.16);if(!civilian&&!car){for(let i=-3;i<=3;i++){const chevron=box(g,.16,.022,.4,fluoro,i*.23,hood+.04,front-.36);chevron.rotation.y=i<0?-.35:.35;}for(let i=-3;i<=3;i++){const stripe=box(g,.18,car?.33:.8,.03,fluoro,i*.27,car?.75:1.26,back-.05);stripe.rotation.z=i<0?-.45:.45;}}
...
if(ambulance){
 ...
 for(let i=-3;i<=3;i++){const stripe=box(g,.16,.028,.55,fluoro,i*.26,hood+.085,front-.36);stripe.rotation.y=i<0?-.48:.48;}
}
