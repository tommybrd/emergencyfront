...
export function tickEquipment(e,minutes){if(!e.hoses)return;if(e.hydrant&&e.supplyHydrant!==e.hydrant){e.supplyHydrant=e.hydrant;e.supplyAnchor=e.hydrant.position.clone();}if(!e.hydrant&&e.supplyProgress===0)e.supplyHydrant=null;for(const h of e.hoses){const wanted=h.index<e.nozzles[h.key];h.progress=Math.max(0,Math.min(1,h.progress+minutes*(wanted?1/({ldt:12,small:20,large:28}[h.key]):-1/15)));}e.supplyProgress=Math.max(0,Math.min(1,(e.supplyProgress||0)+minutes*(e.hydrant?1/20:-1/15)));}
