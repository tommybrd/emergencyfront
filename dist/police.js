...
export function createPolice(world,{advance,actors,release}){
 ...
 for(let i=0;i<2;i++){
  ...
  box(model,1.35,.06,.32,'#24343e',0,1.67,0);
  for(const x of[-.48,.48])model.userData.beacons.push(box(model,.37,.13,.3,new T.MeshStandardMaterial({color:'#518dd0',emissive:'#168aff',emissiveIntensity:0}),x,1.77,0));
 ...
 function lights(t,night){for(const v of units){v.model.userData.headlights.forEach(l=>l.material.emissiveIntensity=night?3:0);v.model.userData.beacons.forEach((b,i)=>b.material.emissiveIntensity=v.call&&Math.sin(t*14+i*Math.PI)>0?5:.05);v.model.userData.light.visible=!!v.call;v.model.userData.light.intensity=night?5:1;}}
