...
export function responseVisuals(world,engines){
 ...
 return{update(t,state){
  ...
  for(const e of engines){
   ...
   team.forEach((p,i)=>{if(['VSAV','VLI'].includes(e.kind))setInterventionHelmet(p,!!onsite);p.visible=!!(onsite&&!e.aerial?.mode&&(rescuing||(recon||medical||diverse)&&!count));if(!p.visible)return;p.position.copy(start).lerp(end,Math.max(0,Math.min(1,phase)));p.position.x+=i?1:-1;p.position.y=0;p.rotation.set(0,Math.atan2(end.x-p.position.x,end.z-p.position.z),0);const walking=phase<1;p.children[1].rotation.x=walking?Math.sin(t*6+i)*.4:0;p.children[2].rotation.x=walking?-Math.sin(t*6+i)*.4:0;});
