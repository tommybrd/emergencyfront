...
export function responseVisuals(world,engines){
 ...
 for(const e of engines){
  ...
  const supply=new T.Line(new T.BufferGeometry().setFromPoints([new T.Vector3(),new T.Vector3()]),new T.LineBasicMaterial({color:'#d8bb64'}));g.add(supply);
  ...
  const team=Array.from({length:e.kind==='VLCG'?0:e.kind==='VLI'?1:2},()=>person(g,0,0,e.kind==='VLI'?'#eeeece':'',e.kind!=='VLI'));
  const kit=box(g,.6,.35,.45,'#e57d38',0,0,0);e.hoseVisuals=lines;
  records.set(e,{g,supply,lines,stretcher,team,kit});
 }
 ...
 return{update(t,state){
  ...
  for(const e of engines){
   const {g,supply,lines,stretcher,team,kit}=records.get(e),c=calls.get(e.call);
   ...
   team.forEach((p,i)=>{p.visible=!!(onsite&&(rescuing||(recon||medical||diverse)&&!count));if(!p.visible)return;p.position.copy(start).lerp(end,Math.max(0,Math.min(1,phase)));p.position.x+=i?1:-1;p.position.y=0;p.lookAt(end);const walking=phase<1;p.children[1].rotation.x=walking?Math.sin(t*6+i)*.4:0;p.children[2].rotation.x=walking?-Math.sin(t*6+i)*.4:0;});
