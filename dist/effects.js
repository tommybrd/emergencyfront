...
export function createIncidentFx(scene,building,scale=[1,1,1],forest=false){
 ...
 return{setActive(v){enabled=v;g.visible=v&&!disposed;},update(t,intensity,spread=0){
  ...
  for(let i=0;i<25;i++){const phase=(t*1.5+i*.18)%1;const width=forest?1+Math.max(0,spread)*2.5:1;dummy.position.set(5+Math.sin(i*13)*5*width,4+phase*7,13.8+Math.cos(i*3)*1.4*width);dummy.scale.set((.4+Math.sin(t*8+i)*.2)*intensity,(1.3-phase*.8)*intensity,.45*intensity);dummy.updateMatrix();fire.setMatrixAt(i,dummy.matrix);}fire.instanceMatrix.needsUpdate=true;orange.intensity=intensity*(90+Math.sin(t*9)*30);
