import {pumpCapacity} from './hydraulics.js';
...
export function vehicleConsole(e,{boarding=false,autoSiren=false,follow=false,returnLabel='Retour au CIS',incident=null,engines=[],supply=null,lakeSupply=null}={}){
 ...
 const modes=e.model.userData.rearAmber?.length>=8?`<div class="amberModes" role="group" aria-label="Sens de la rampe orange vus depuis l’arrière"><button type="button" class="signalKey amberKey offKey" data-amber-mode="off" title="Éteindre la rampe orange" aria-label="Rampe arrière · OFF" aria-pressed="${!amber}">OFF</button>${AMBER_PATTERNS.map(([mode,label])=>key({action:`data-amber-mode="${mode}"`,icon:mode,label:'Rampe arrière · '+label,color:'amberKey',on:amber&&(e.amberPattern||'alternate')===mode})).join('')}</div>`:e.model.userData.rearAmber?.length?`<div class="amberModes"><button data-amber-mode="off" aria-pressed="${!amber}">OFF</button><button data-amber-mode="flash" aria-pressed="${!!amber}">Orange</button></div>`:'';
 const pump=fire?`<div class="pumpScreen">${waterInstrument(e)}<p class="nozzleCrew">Pompe · ${pumpCapacity(e).toLocaleString('fr-FR')} L/min max${e.pumpLimited?' · Débit partagé à la limite de pompe':''}</p><p class="nozzleCrew">${nozzleCount(e)} lance(s) demandée(s) · ${nozzleLimit(e)} binôme(s) disponible(s)${nozzleCount(e)>nozzleLimit(e)?' · Lances en attente d’effectif':''}</p><div class="nozzleControls" role="group" aria-label="Lances à déployer">${Object.entries(NOZZLES).map(([id,n])=>`<div class="nozzleRow ${id}" title="${n.label} · ${n.flow} L/min par lance"><span aria-label="${n.label}">${consoleIcon('nozzle')}</span>${Array.from({length:n.max+1},(_,count)=>`<button type="button" data-nozzle="${id}" data-number="${count}" title="${count} ${n.label.toLowerCase()} · ${count*n.flow} L/min" aria-label="${count} ${n.label.toLowerCase()}" ${!scene||!!e.noria||!nozzleAllowed(e,id,count)||e.tacticalPlacement==='Aspiration au lac'&&count>0?'disabled':''} aria-pressed="${e.nozzles[id]===count}">${count}</button>`).join('')}</div>`).join('')}</div></div>`:'';
 const navigation=[
 ...
 return `<div class="signalController ${e.kind==='EPA'?'aerialController':''}"><i class="caseScrew screwTL" aria-hidden="true"></i><i class="caseScrew screwTR" aria-hidden="true"></i><div class="signalScreen"><div class="consoleButtons" role="group" aria-label="Commandes embarquées">${buttons.map(key).join('')}</div>${modes}<div class="sirenModes" role="group" aria-label="Volume du deux-tons"><span>Deux-tons</span>${[['auto','Auto','Volume réduit de 21 h à 7 h'],['day','Jour','Volume de jour'],['night','Nuit','Volume réduit']].map(([mode,label,hint])=>`<button type="button" data-siren-mode="${mode}" aria-pressed="${(e.sirenMode||'auto')===mode}" title="${hint}">${label}</button>`).join('')}</div>${pump}${e.aerial?.mode?`<div class="aerialReadout" role="status">${consoleIcon(e.aerial.mode==='attack'?'aerialNozzle':'stretcher')}<span>${escapeHtml(aerialStatus(e))}${e.aerial.flow>0?` · ${Math.round(e.aerial.flow)} L/min`:e.aerial.phase==='pack'?'':` · ${Math.round(e.aerial.progress*100)} %`}${e.aerial.sourceId?`<small>${escapeHtml(e.aerial.sourceId)}</small>`:''}</span></div>`:''}</div><div class="signalFooter" role="group" aria-label="Navigation">${navigation.map(key).join('')}</div><i class="caseScrew screwBL" aria-hidden="true"></i><i class="caseScrew screwBR" aria-hidden="true"></i></div>`;
}
