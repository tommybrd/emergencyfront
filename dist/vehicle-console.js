...
const symbols={
 ...
 unblock:'<path d="M17 3 2 27h30zM17 11v7m0 4v1"/>',
...
export function vehicleConsole(e,{boarding=false,autoSiren=false,follow=false,returnLabel='Retour au CIS'}={}){
 ...
 const buttons=[
  ...
  ...(e.kind==='EPA'?[{action:'data-ladder',icon:'ladder',label:'Déployer ou replier l’échelle',color:'workKey',on:e.ladderDeployed,disabled:!scene}]:[]),
 ...
 const navigation=[
  ...
  {action:'data-cis',icon:'station',label:returnLabel,disabled:['ready','hospital','transport','reconditioning','returning'].includes(e.status)},
  ...(e.trafficWaiting&&e.path?[{action:'data-unblock',icon:'unblock',label:'Voie occupée · tenter de débloquer le trajet',color:'warningKey',on:true}]:[])
 ];
 return `<div class="signalController"><i class="caseScrew screwTL" aria-hidden="true"></i><i class="caseScrew screwTR" aria-hidden="true"></i><div class="signalScreen"><div class="consoleButtons" role="group" aria-label="Commandes embarquées">${buttons.map(key).join('')}</div>${modes}${pump}<div class="signalVoltage" aria-label="Alimentation 12 volts">${consoleIcon('battery')}<span class="voltageBars" aria-hidden="true">▮▮▮▮▮▮▮▮</span><span>12 V</span></div></div><div class="signalFooter" role="group" aria-label="Navigation">${navigation.map(key).join('')}</div><i class="caseScrew screwBL" aria-hidden="true"></i><i class="caseScrew screwBR" aria-hidden="true"></i></div>`;
}
