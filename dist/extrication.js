...
export const rescueTruck=e=>e.kind==='FPT'&&!e.lightPump;
export function extricationStatus(c){const r=c?.extrication;if(!r||!c.reconComplete)return '';return r.done?'Victime dégagée · relais VSAV':!r.unitId?'Victime coincée · FPTSR nécessaire':r.approach<1?'Préparation des outils':r.progress<1?'Désincarcération · '+Math.round(r.progress*100)+' %':'Victime dégagée · rangement des outils';}
export function tickExtrications(calls,engines,minutes,emit){
 ...
 for(const c of calls){const r=c.extrication;if(!r||r.done||!c.reconComplete||c.status==='closed')continue;
  ...
  if(!unit){r.unitId=null;r.approach=0;if(!r.requested){r.requested=true;emit(c,'Centre','Victime coincée. Renfort FPTSR demandé.',true);}continue;}
