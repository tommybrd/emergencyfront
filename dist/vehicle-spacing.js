...
export function footprint(model,x=model.position.x,z=model.position.z,yaw=model.rotation.y){if(model.userData.staticFootprint)return {x,z,yaw,...model.userData.staticFootprint};const scale=model.scale?.x||1,kind=model.userData.kind;return{x,z,yaw,length:((model.userData.length||4.45)+.8)*scale,width:((['FPT','CCF','EPA','CCGC'].includes(kind)?3.15:kind==='VLCG'?2.25:2.6)+.3)*scale};}
