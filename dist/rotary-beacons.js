...
export function updateAmberEffects(model,enabled,pattern,now,night){
 ...
 if(model.userData.signalAmber==='round'){const a=now*.007;for(const [i,{lamp,glow}]of rig.lamps.entries()){const strength=enabled?Math.pow(Math.max(0,Math.cos(a+i*Math.PI)),6):0;lamp.material.emissiveIntensity=enabled?.15+5*strength:.03;glow.visible=!!enabled;glow.material.opacity=strength*(night?.8:.5);}rig.beam.visible=!!enabled;rig.beam.intensity=enabled?(night?90:32):0;rig.beam.target.position.set(rig.beam.position.x+Math.sin(a)*7,.2,rig.beam.position.z+Math.cos(a)*7);return;}
