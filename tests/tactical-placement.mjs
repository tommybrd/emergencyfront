...
function advance(){const before=new Map(game.vehicleObstacles().map(m=>[m,m.position.clone()]));state.minute+=.25;tickEngines(.25);ticks++;
 ...
 for(let i=0;i<models.length;i++){if(engines.some(e=>e.model===models[i])){const jumped=models[i].position.distanceTo(before.get(models[i]))>=4.1,safetyReset=state.logs.at(-1)?.message.includes('retour de sécurité au CIS');assert(!jumped||safetyReset,'No instant responder repositioning outside the explicit hard reset');}for(let j=i+1;j<models.length;j++)assert(!overlaps(footprint(models[i]),footprint(models[j])),'Vehicles stay separate');}
