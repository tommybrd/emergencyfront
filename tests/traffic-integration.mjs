...
function advance(){const previous=new Map(game.vehicleObstacles().filter(m=>m.visible!==false).map(m=>[m,m.position.clone()]));state.minute+=.25;tickEngines(.25);ticks++;
 ...
 for(let i=0;i<models.length;i++){const m=models[i];if(previous.has(m)&&m.position.distanceTo(previous.get(m))>=4.1)console.log({ticks,before:previous.get(m),after:m.position,logs:state.logs.slice(-4),vehicles:checked.map(e=>({id:e.id,p:[e.model.position.x,e.model.position.z],seg:e.segment,status:e.status,wait:e.controlWaiting,blocked:e.blockedSeconds}))});assert(!previous.has(m)&&!engines.some(e=>e.model===m)||previous.has(m)&&m.position.distanceTo(previous.get(m))<4.1,'Unexpected repositioning '+(engines.find(e=>e.model===m)?.id||'civilian'));
