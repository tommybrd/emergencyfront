...
selectIncident(1);assert.equal(engageUnits(['FPTSR','CCF 1','CCF 2','EPA']),null);
selectIncident(2);assert.equal(engageUnits(['VSAV 1','VSAV 2','VSAV 3','VSAV 4','VTU','VLCG']),null);
const checked=engines.filter(e=>!['VPL','VLI'].includes(e.kind)&&!['FPTL 1','CCF 3'].includes(e.id));let ticks=0,maxWait=0;
function advance(){const previous=new Map(game.vehicleObstacles().map(m=>[m,m.position.clone()]));state.minute+=.25;tickEngines(.25);ticks++;
 const models=game.vehicleObstacles().filter(m=>m.visible!==false);
 for(let i=0;i<models.length;i++){const m=models[i];if(m.position.distanceTo(previous.get(m))>=4.1)console.log({ticks,before:previous.get(m),after:m.position,logs:state.logs.slice(-4),vehicles:checked.map(e=>({id:e.id,p:[e.model.position.x,e.model.position.z],seg:e.segment,status:e.status,wait:e.controlWaiting,blocked:e.blockedSeconds}))});assert(m.position.distanceTo(previous.get(m))<4.1,'Unexpected repositioning '+(engines.find(e=>e.model===m)?.id||'civilian'));
...
until(()=>engines.find(e=>e.id==='CCF 2').status==='ready',400,'CCF 8000 returns');
selectIncident(1);assert.equal(engageUnits(['CCF 2']),null);
until(()=>checked.every(e=>e.id==='CCF 2'?e.status==='scene':e.status==='ready'),500,'Concurrent return and departure clear the station');
returnEngine(engines.find(e=>e.id==='CCF 2'));
...
const forest={id:3,type:'INC',name:'Feu de végétation',requires:'CCF',at:state.minute,status:'waiting',duration:120,progress:0};state.calls.push(forest);onCall(forest);selectIncident(3);assert.equal(engageUnits(['CCF 1','CCF 2']),null);
...
until(()=>engines.find(e=>e.id==='CCF 2').path?.some(p=>p.gear===-1)&&engines.find(e=>e.id==='CCF 2').reversing,500,'CCF 8000 starts reverse parking');
selectIncident(3);assert.equal(engageUnits(['CCF 2']),null);
until(()=>engines.find(e=>e.id==='CCF 2').status==='scene'&&engines.find(e=>e.id==='CCF 1').status==='ready',500,'Redispatch during reverse parking');
