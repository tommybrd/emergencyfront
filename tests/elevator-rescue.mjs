...
for(const medicalChance of[1,0]){
 ...
 assert(c.elevator);assert.equal(c.victimCount,0);assert(!capability(vsav,c));selectIncident(c.id);assert.equal(engageUnits([vtu.id]),null);
 ...
 if(medicalChance){
  ...
  assert(c.reinforcementAlerts.some(a=>a.need==='vsav'),'VSAV requested in incident panel');
  selectIncident(c.id);assert.equal(engageUnits([vsav.id]),null);
...
console.log('PASS occasional elevator casualty, no dispatch spoiler, reconnaissance, single VSAV request, release before loading, CH closure and uncomplicated rescue');
