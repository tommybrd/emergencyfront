...
let seed=73;Math.random=()=>{seed=(Math.imul(seed,1664525)+1013904223)>>>0;return seed/4294967296;};
...
state.schedule=[];state.next=0;state.shiftEnd=100000;state.minute=1320;
...
const called=requestVolunteers(state,3);assert(called>0);const phases=new Set();let steps=0;
function step(){
...
step();assert.equal(state.freeStaff,11);assert(volunteerTravel.records.size>0);assert(state.recallRequests.some(r=>r.activity==='home'));
...
console.log('PASS initial general night recall without collisions');
